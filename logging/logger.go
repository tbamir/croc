...
func (l *Logger) LogTransfer(log TransferLog) error {
	...
	fileHash := ""
	if log.Status == "success" && log.Direction == "receive" {
		// For received files, we could calculate hash here if needed
		// For now, we'll use a placeholder
		fileHash = generateFileHash(log.FileName, log.FileSize)
	}
...
// generateFileHash generates a placeholder hash for a file
// In production, this would calculate actual file hash
func generateFileHash(fileName string, fileSize int64) string {
	data := fmt.Sprintf("%s-%d", fileName, fileSize)
	hash := sha256.Sum256([]byte(data))
